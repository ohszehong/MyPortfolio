...
export function AIsCollidedWithB(collisionA, collisionB, collisionType = "AABB")
{
    if(collisionType === "AABB")
    {
        ...
        else
        {
            _collisionA = {
                ...
                dxEnd: collisionA.source.position.dx + collisionA.ddx + collisionA.width,
                dyEnd: collisionA.source.position.dy + collisionA.ddy + collisionA.height
            };
        }
        ...
        else
        {
            _collisionB = {
                ...
                dxEnd: collisionB.source.position.dx + collisionB.ddx + collisionB.width,
                dyEnd: collisionB.source.position.dy + collisionB.ddy + collisionB.height
            };
        }
...
export function PawnActorIsOnTrigger(actor, trigger)
{
    ...
    const actorBottomLeftDx = actorCollision.source.position.dx + actorCollision.ddx;
    const actorBottomLeftDy = actorCollision.source.position.dy + actorCollision.ddy + actorCollision.height;
...
function isEligible(A, B)
{
    ...
    switch(A.target)
    {
        case TargetTypes.all:
            if(B.target != TargetTypes.none) eligible = true;
            break;

        case TargetTypes.ally:
            if(B.target === TargetTypes.all || B.target === TargetTypes.ally) eligible = true;
            break;

        case TargetTypes.enemy:
            if(B.target === TargetTypes.all || B.target === TargetTypes.enemy) eligible = true;
            break;
    }
