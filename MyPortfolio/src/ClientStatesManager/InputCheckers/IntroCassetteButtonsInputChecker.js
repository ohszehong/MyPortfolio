...
/** @param {StatesManager} clientManager */
export default function handleKeys(clientManager)
{
  let lastKeys = clientManager.lastKeys;
  const keys = clientManager.keys;
  ...
  for(let key in keys)
  {
    if(keys[key])
    {
      ...
      clientManager.sendMessageToServer(SocketMessageTypes.userInput, key);
