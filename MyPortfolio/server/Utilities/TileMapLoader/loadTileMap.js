...
export default async function loadTileMap(shouldAbortRef) {
  ...
  try {
    for (let tileset of mapJSON.tilesets) {
      ...
      if (tileset.tiles) {
        tileset.tiles.forEach((tileProperties) => {
          ...
          const renderLast = tileProperties.properties?.find(
            (property) => property.name === "renderLast",
          );
          if (renderLast) {
        ...
        if (decodedTile.gid - 1 > 0) {
          ...
          if (tileWithProperties) {
            ...
            if (tileWithProperties.animation?.length > 0) {
              const firstGid =
                decodedTile.gid - tileWithProperties.animation[0].tileid;
              ...
              tileWithProperties.animation.forEach((frameData) => {
                ...
                const tileData = tilesWithProperties[gid]
                  ? tilesWithProperties[gid]
                  : tilesWithoutProperties[gid];
            ...
            const dy = layer.y + currentTileRowInIndex * tileHeightInPixels; //don't have to offset with tileWithProperties.tileCanvas.height here because currentTileColumnInIndex starts from 0 which mean we already shift the position by 16pixels by default.
                  ...
                  if (tileBlob) {
                    ...
                    if (
                      !this.tileActorsBlobDictionary[
                        currentTileData.tiles[i].tileGid
                      ]
                    ) {
                      this.tileActorsBlobDictionary[
                        currentTileData.tiles[i].tileGid
    ...
    const soundTriggersLayer = mapJSON.layers.find(
      (layer) => layer.name === "SoundTriggers",
    );
    ...
    //for DefenseMarch
    const summonLocations = mapJSON.layers.find(
      (layer) => layer.name === "SummonLocations",
    );
    if (summonLocations) {
      const allySummonLocations = summonLocations.objects.filter(
        (locations) => locations.for === "ally",
      );
      if (allySummonLocations) {
        this.allySummonLocations = [...allySummonLocations];
      }

      const enemySummonLocations = summonLocations.objects.filter(
        (locations) => locations.for === "enemy",
      );
